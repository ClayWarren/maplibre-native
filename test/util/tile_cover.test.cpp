...
TEST(TileCover, Arctic) {
    EXPECT_EQ((std::vector<UnwrappedTileID>{}), util::tileCover(LatLngBounds::hull({86, -180}, {90, 180}), 0));
}
...
TEST(TileCover, HighPitch) {
    ...
    util::TileCoverParameters params{transform.getState()};
    params.tileLodPitchThreshold = 0.0;
    params.tileLodMinRadius = 1.0;
    EXPECT_EQ(
...
TEST(TileCover, PitchWithLargerResultSet) {
    ...
    auto cover = util::tileCover({.transformState = transform.getState()}, 5, zoomRange);
...
TEST(TileCover, DifferentOverscaledZWithPitch) {
    ...
    EXPECT_EQ((std::vector<OverscaledTileID>{{5, 0, {3, 3, 3}},
                                             ...
                                             {5, 0, {3, 4, 1}},
                                             {5, 0, {3, 2, 1}}}),
              util::tileCover(params, 3, zoomRange, 5));
}
...
TEST(TileCover, SanFranciscoZ0Wrapped) {
    EXPECT_EQ((std::vector<UnwrappedTileID>{{0, 1, 0}}), util::tileCover(sanFranciscoWrapped, 0));
}

TEST(TileCover, GeomPoint) {
    auto point = Point<double>{-122.5744, 37.6609};

    EXPECT_EQ((std::vector<UnwrappedTileID>{{2, 0, 1}}), util::tileCover(point, 2));
}

TEST(TileCover, GeomMultiPoint) {
    auto points = MultiPoint<double>{{-122.5, 37.76}, {-122.4, 37.76}};

    EXPECT_EQ((std::vector<UnwrappedTileID>{{20, 167480, 405351}, {20, 167772, 405351}}), util::tileCover(points, 20));
}
...
TEST(TileCover, GeomSanFranciscoPoly) {
    auto sanFranciscoGeom = Polygon<double>{
        {{-122.5143814086914, 37.779127216982424},  {-122.50811576843262, 37.72721239056709},
         {-122.50313758850099, 37.70820178063929},  {-122.3938751220703, 37.707454835665274},
         {-122.37567901611328, 37.70663997801684},  {-122.36297607421874, 37.71343018466285},
         {-122.354736328125, 37.727280276860036},   {-122.36469268798828, 37.73868429065797},
         {-122.38014221191408, 37.75442980295571},  {-122.38391876220702, 37.78753873820529},
         {-122.35919952392578, 37.8065289741725},   {-122.35679626464844, 37.820632846207864},
         {-122.3712158203125, 37.835276322922695},  {-122.3818588256836, 37.82958198283902},
         {-122.37190246582031, 37.80788523279169},  {-122.38735198974608, 37.791337175930686},
         {-122.40966796874999, 37.812767557570204}, {-122.46425628662108, 37.807071480609274},
         {-122.46803283691405, 37.810326435534755}, {-122.47901916503906, 37.81168262440736},
         {-122.48966217041016, 37.78916666399649},  {-122.50579833984375, 37.78781006166096},
